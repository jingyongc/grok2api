...
PATCHED VERSION (local override):
  Fix: When xAI rate-limits API doesn't return a particular mode's quota
  for an account on super/heavy pool, the original code leaves the field
  as DEFAULT 0/0 forever (because the empty default window never satisfies
  ``is_window_expired``). This blocks video/expert/grok_4_3 routing even
  for legitimately-subscribed super accounts.

  Patch behaviour: if a supported mode is missing from upstream response
  and local state is DEFAULT with total=0, populate it with the pool's
  built-in defaults so account lease decisions don't permanently exclude it.
"""
...
def _maybe_fill_pool_default(
    ...
    """If a pool-supported mode is uninitialised (DEFAULT + total=0), seed it
    with the pool's built-in default so it can participate in lease decisions.

    Returns True when a patch was applied for this mode (caller should skip
    the original DEFAULT/REAL branches for this mode).
    """
    if (
        existing.source != QuotaSource.REAL
        and existing.total == 0
        and supports_mode(record.pool, mode_id)
    ):
        default = default_quota_window(record.pool, mode_id)
        if default is not None and default.total > 0:
            ...
            logger.info(
                "account quota seeded from pool default: token={}... pool={} mode_id={} total={}",
                record.token[:10],
                record.pool,
                mode_id,
                default.total,
...
class AccountRefreshService:
        ...
        *,
        ...
        for mode in ALL_MODES_FULL:
            ...
            if mode_id in windows:
                ...
                if existing is None:
                    continue
                # PATCH: pool supports this mode but quota was never initialised
                # (e.g. account joined as basic then manually promoted to super).
                # Seed with pool default instead of leaving 0/0 forever.
                if _maybe_fill_pool_default(record, mode_id, existing, patches, now):
                    continue
    ...
    async def _apply_fallback(self, record: AccountRecord) -> RefreshResult:
        ...
        for mode in ALL_MODES_FULL:
            ...
            if existing is None:
                continue
            # PATCH: same uninitialised-mode seed as in _refresh_one.
            if _maybe_fill_pool_default(record, mode_id, existing, patches, now):
                continue
            if existing.source == QuotaSource.REAL:
                patches[_MODE_KEYS[mode_id]] = QuotaWindow(
